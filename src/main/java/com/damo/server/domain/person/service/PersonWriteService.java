...
public class PersonWriteService {
    ...
    public void addPerson(final RequestCreatePersonDto personDto) {
        ...
        if(personRepository.existsByNameAndRelationAndUserId(personDto.name(), personDto.relation(), securityUserUtil.getId())) {
    ...
    public void patchPersonById(final RequestUpdatePersonDto personDto, final Long personId) {
        final Person person = personRepository.findByIdAndUserId(personId, securityUserUtil.getId()).orElseThrow(() -> new CustomException(CustomErrorCode.NOT_FOUND, "수정할 대상을 찾을 수 없음"));
        person.changeInfo(personDto);
    }

    @Transactional
    public void removePersonById(final Long personId) {
        personRepository.findByIdAndUserId(personId, securityUserUtil.getId()).orElseThrow(() -> new CustomException(CustomErrorCode.NOT_FOUND, "삭제할 대상을 찾을 수 없음"));
        personRepository.deleteByIdAndUserId(personId, securityUserUtil.getId());
    }
}
