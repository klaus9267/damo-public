...
public interface PersonRepository extends JpaRepository<Person, Long> {
    ...
    @Query("SELECT new com.damo.server.domain.person.dto.PeopleWithScheduleCountDto(p, COUNT(s)) FROM Person p LEFT JOIN p.schedules s WHERE p.user.id = :userId AND (:relation IS NULL OR p.relation = :relation) GROUP BY p.id")
