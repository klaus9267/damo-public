...
/**
 * {@code ScheduleReadOperation} 어노테이션은 조회된 데이터 응답을 설명하는 OpenAPI Operation을 정의합니다.
 */
...
@Operation(
    responses = {
        ...
        @ApiResponse(
            ...
            content = @Content(
                examples = {@ExampleObject(name = "잘못된 일정 ID", value = "{\"statusCode\" : 400, \"statusMessage\" : \"Bad Request\", \"message\" : \"bad request\" }")},
        ...
        @ApiResponse(
            ...
            content = @Content(
                examples = {@ExampleObject(name = "조회할 일정 없음", value = "{\"statusCode\" : 404, \"statusMessage\" : \"Unauthorized\", \"message\" : \"조회할 일정을 찾을 수 없음\" }")},
                schema = @Schema(implementation = ResponseCustomException.class)
            )
        ),
        @ApiResponse(
...
public @interface ScheduleReadOperation {
  /**
