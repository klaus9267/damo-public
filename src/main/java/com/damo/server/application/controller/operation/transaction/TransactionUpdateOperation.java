...
/**
 * {@code TransactionUpdateOperation} 어노테이션은 데이터를 수정하는 OpenAPI Operation을 정의합니다.
 */
...
@Operation(
    ...
    responses = {
        ...
        @ApiResponse(
            ...
            content = @Content(
                examples = {
                    @ExampleObject(name = "잘못된 transaction id", value = "{\"statusCode\" : 400, \"statusMessage\" : \"Bad Request\", \"message\" : \"bad request\" }"),
        ...
        @ApiResponse(
            ...
            content = @Content(
                examples = {@ExampleObject(name = "수정할 내역 없음", value = "{\"statusCode\" : 404, \"statusMessage\" : \"Unauthorized\", \"message\" : \"수정할 내역을 찾을 수 없음\" }")},
                schema = @Schema(implementation = ResponseCustomException.class)
            )
        ),
        @ApiResponse(
...
public @interface TransactionUpdateOperation {
  /**
