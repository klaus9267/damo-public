...
/**
 * {@code ScheduleCreateOperation} 어노테이션은 일정 정보 생성 관련 작업을 설명하는 OpenAPI Operation을 정의합니다.
 */
@Target({ElementType.METHOD, ElementType.ANNOTATION_TYPE})
@Retention(RetentionPolicy.RUNTIME)
@Operation(
    requestBody = @RequestBody(content = @Content(schema = @Schema(implementation = RequestCreateScheduleDto.class))),
    responses = {
        ...
        @ApiResponse(
            ...
            content = @Content(
                examples = {@ExampleObject(name = "동일 행사, 동일 날짜 입력", value = "{\"statusCode\" : 400, \"statusMessage\" : \"Bad Request\", \"message\" : \"일정 내에서 동일한 기록이 존재\" }")},
...
public @interface ScheduleCreateOperation {
  /**
