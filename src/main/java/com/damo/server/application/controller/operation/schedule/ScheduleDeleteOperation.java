...
/**
 * {@code ScheduleDeleteOperation} 어노테이션은 일정 정보 삭제 관련 작업을 설명하는 OpenAPI Operation을 정의합니다.
 */
...
@Operation(
    responses = {
        ...
        @ApiResponse(
            ...
            content = @Content(
                examples = {@ExampleObject(name = "삭제할 일정 없음", value = "{\"statusCode\" : 404, \"statusMessage\" : \"Unauthorized\", \"message\" : \"삭제할 일정을 찾을 수 없음\" }")},
                schema = @Schema(implementation = ResponseCustomException.class)
            )
        ),
        @ApiResponse(
...
public @interface ScheduleDeleteOperation {
  /**
