...
/**
 * {@code TransactionReadOperation} 어노테이션은 조회된 데이터 응답을 설명하는 OpenAPI Operation을 정의합니다.
 */
...
@Operation(
    responses = {
        @ApiResponse(
            ...
            content = @Content(schema = @Schema(implementation = TransactionReadOperation.class))
        ),
        @ApiResponse(
            responseCode = "400",
            ...
            content = @Content(
                examples = {
                    @ExampleObject(name = "잘못된 쿼리 스트링", value = "{\"statusCode\" : 400, \"statusMessage\" : \"Bad Request\", \"message\" : \"bad request\" }")},
...
public @interface TransactionReadOperation {
